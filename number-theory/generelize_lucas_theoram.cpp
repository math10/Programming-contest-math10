...
int64 generelize_lucas_theoram(int64 n,int64 r,int64 p,int64 q){
    ...
    rep(i,n_len) ans = (ans*fact[N_js[i]])%mod;
    rep(i,m_len) ans = (ans*modInverse_relativePrime(fact[M_js[i]],mod))%mod;
    rep(i,r_len) ans = (ans*modInverse_relativePrime(fact[R_js[i]],mod))%mod;
